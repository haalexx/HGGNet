...
class HGGNet(nn.Module):
    def __init__(self, model_configs):
        ...
        self.up32 = FPT_UpsampleBlock(in_channels=384, fp_mlp=[256, 256], emb_dim=256)
    ...
    def forward(self, ptc):
        ...
        q = self.mlp_query(
            torch.cat([
                global_feature.unsqueeze(1).expand(-1, coarse_3.size(1), -1),
                coarse_3], dim = -1)).transpose(2,1).contiguous() # b c n
