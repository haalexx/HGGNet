...
class HGGNet(nn.Module):
    def __init__(self, model_configs):
        ...
        self.self_encoder = TransformerEncoder(
            embed_dim=384, depth=4, num_heads=4, k=8, mlp_ratio=2, combine_style='concat',
        ...
        self.down23 = EdgeSA(in_channel=256, out_channel=384, k=8)
        ...
        self.rebuild1 = SimpleRebuildFCLayer(384, step=16, hidden_dim=512)
        self.rebuild2 = SimpleRebuildFCLayer(384, step=16, hidden_dim=512)
        self.rebuild3 = SimpleRebuildFCLayer(384, step=16, hidden_dim=512)
    ...
    def forward(self, ptc):
        ...
        coarse = torch.gather(coarse, 1, idx[:, :1024].expand(-1, -1, coarse.size(-1)))

        coarse_3 = coarse[:, :128, :].contiguous()
        coarse_2 = coarse[:, 128:448, :].contiguous()
        coarse_1 = coarse[:, 448:, :].contiguous()
        ...
        s12_down = self.down12(s1_fuse, coarse_1, num=320)
