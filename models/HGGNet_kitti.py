...
class FPT_UpsampleBlock(nn.Module):
    ...
    def forward(self, up_xyz, xyz, up_features, features, global_features):
...
class HGGNet(nn.Module):
    ...
    def forward(self, ptc):
        ...
        edge_feature, down_ptc = self.edge_downsample(input_feature, ptc)
        ...
        s32 = self.up32(stage2_center, stage3_center, stage2_feature, s3, global_features)
        s2 = self.conv2(stage2_feature)
        s2_fuse1 = self.featureFuse1(s32, s2)
        s2_fuse1 = self.conv3(s2_fuse1)
        s21 = self.up21(stage1_center, stage2_center, stage1_feature, s2_fuse1, global_features)
        ...
        center2_xyz = fps_sample_points(self.stage2_npoints, center1_xyz)
        center3_xyz = fps_sample_points(self.stage3_npoints, center2_xyz)
        ...
        out_npoints = rebuild_points.shape[1]
        out_points = fps_sample_points(out_npoints - npoints, rebuild_points)
        out_points = torch.cat([out_points, ptc], dim=1).contiguous()

        ret = (center_xyz, out_points)
...
if __name__ == "__main__":
    ...
    test_ptc = torch.rand(size=(4, 3, 2048), dtype=torch.float32).to(device)
    test_fea = torch.rand(size=(4, 8, 2048), dtype=torch.float32).to(device)
    config = cfg_from_yaml_file("../cfgs/model_configs/HFFNet_v1.yaml")
