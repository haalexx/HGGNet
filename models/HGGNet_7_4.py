...
class FPT_UpsampleBlock(nn.Module):
    def __init__(self, in_channels, fp_mlp, emb_dim):
        ...
        self.transformer = TransformerDecoder(
            embed_dim=emb_dim, depth=3, num_heads=6, k=8, n_group=2, mlp_ratio=2,
    ...
    def forward(self, up_xyz, xyz, features, mem, xyz1):
        ...
        # v = self.mem_proj(mem)
        out_features = self.transformer(q=new_features, v=mem, q_pos=up_xyz, v_pos=xyz1, denoise_length=None)
...
class HGGNet(nn.Module):
    def __init__(self, model_configs):
        ...
        self.dectransformer = TransformerDecoder(
            embed_dim=384, depth=3, num_heads=6, k=8, n_group=2, mlp_ratio=2,
            self_attn_block_style_list=['attn-graph', 'attn', 'attn'],
            self_attn_combine_style='concat',
            cross_attn_block_style_list=['attn-graph', 'attn', 'attn'],
            cross_attn_combine_style='concat'
        ...
        self.down12 = EdgeSA(in_channel=384, out_channel=384, k=8)
        self.featureFuse3 = CAFFBlock(channel=384, channel2=384, out_channel=384)

        self.down23 = EdgeSA(in_channel=384, out_channel=384, k=8)
        self.featureFuse4 = CAFFBlock(channel=384, channel2=384, out_channel=384)

        self.denoise_feautre = EdgeSA(in_channel=384, out_channel=384,k=8)

        self.reduce_dim1 = nn.Linear(1027 + 384, 384)
        self.reduce_dim2 = nn.Linear(1027 + 384, 384)
    ...
    def get_loss(self, ret, gt, epoch=1):
        ...
        # denoise loss
        idx = knn_point(32, gt, denoised_coarse) # B n k
    ...
    def forward(self, ptc):
        ...
        if self.training:
            picked_points = fps(ptc, 200)
            picked_points = jitter_points(picked_points)
            picked_points_3 = picked_points[:, :48, :].contiguous()
            picked_points_2 = picked_points[:, 48:112, :].contiguous()
            picked_points_1 = picked_points[:, 112:, :].contiguous()

            denoise_q = self.mlp_query(
                torch.cat([
                global_feature.expand(-1, 48, -1),
                picked_points_3], dim = -1)
            )
            denoise_q = self.dectransformer(q=denoise_q, v=x, q_pos=picked_points_3, v_pos=stage_center).transpose(2,1).contiguous() # b c n

            s2_denoise = self.up32(picked_points_2, picked_points_3, denoise_q, mem=x, xyz1=stage_center)
            s2_denoise = s2_denoise.transpose(1, 2).contiguous()

            s1_denoise = self.up21(picked_points_1, picked_points_2, s2_denoise, mem=x, xyz1=stage_center)
            s1_denoise_t = s1_denoise.transpose(1, 2).contiguous()

            s12_denoise_down = self.down12(s1_denoise_t, picked_points_1, num=64)
            s2_denoise_fuse = self.featureFuse3(s12_denoise_down, s2_denoise)

            s23_denoise_down = self.down23(s2_denoise_fuse, picked_points_2, num=48)

            denoise_feautre = self.denoise_feautre(feature, stage_center, num=48)

            s3_denoise_fuse = self.featureFuse4(s23_denoise_down, denoise_feautre)

            head1_denoise = s1_denoise
            head2_denoise = s2_denoise_fuse.transpose(2, 1).contiguous()
            head3_denoise = s3_denoise_fuse.transpose(2, 1).contiguous()

            rebuild3_feature_denoise = torch.cat([
                global_feature.expand(-1, 48, -1),
                head3_denoise,
                picked_points_3], dim=2)  # B M 1027 + C
            rebuild3_feature_denoise = self.reduce_dim3(rebuild3_feature_denoise)  # B M C
            xyz3_denoise = self.rebuild3(rebuild3_feature_denoise).reshape(bs, 48, 3, -1)  # B M 3 S
            rebuild_denoise_3 = xyz3_denoise.reshape(bs, -1, 3)  # B N 3

            rebuild2_feature_denoise = torch.cat([
                global_feature.expand(-1, 64, -1),
                head2_denoise,
                picked_points_2], dim=2)  # B M 1027 + C
            rebuild2_feature_denoise = self.reduce_dim2(rebuild2_feature_denoise)  # B M C
            xyz2_denoise = self.rebuild2(rebuild2_feature_denoise).reshape(bs, 64, 3, -1)  # B M 3 S
            rebuild_denoise_2 = xyz2_denoise.reshape(bs, -1, 3)  # B N 3

            rebuild1_feature_denoise = torch.cat([
                global_feature.expand(-1, 88, -1),
                head1_denoise,
                picked_points_1], dim=2)  # B M 1027 + C
            rebuild1_feature_denoise = self.reduce_dim1(rebuild1_feature_denoise)  # B M C
            xyz1_denoise = self.rebuild1(rebuild1_feature_denoise).reshape(bs, 88, 3, -1)  # B M 3 S
            rebuild_denoise_1 = xyz1_denoise.reshape(bs, -1, 3)  # B N 3

            # cat the input
            rebuild_denoise = torch.cat([rebuild_denoise_1, rebuild_denoise_2, rebuild_denoise_3], dim=1).contiguous()
            

        q = self.mlp_query(
            torch.cat([
                global_feature.expand(-1, coarse_3.size(1), -1),
                coarse_3], dim = -1)
        )

        q = self.dectransformer(q=q, v=x, q_pos=coarse_3, v_pos=stage_center, denoise_length=None).transpose(2,1).contiguous() # b c n

        s2 = self.up32(coarse_2, coarse_3, q, mem=x, xyz1=stage_center)  # e
        s2 = s2.transpose(1, 2).contiguous()

        s1 = self.up21(coarse_1, coarse_2, s2, mem=x, xyz1=stage_center)
        ...
        if self.training:
            ret = (coarse, rebuild_points, picked_points, rebuild_denoise)
        else:
