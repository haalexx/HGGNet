...
class UpBlock(nn.Module):
    def __init__(self, feat_dim=512, down_rates=[1, 4, 2], knns=[16, 12, 8], up_factor=4):
        super().__init__()
        self.pn = PN(feat_dim=384, global_dim=384, out_dim=128)
...
class Decoder(nn.Module):
    def __init__(self, encoded_feature_dim=384):
        ...
        self.head_fp1 = PointNet_FP_Module(in_channel=384, mlp=[256, 384], use_points1=False)
        self.head_fp2 = PointNet_FP_Module(in_channel=384, mlp=[256, 384], use_points1=False)
        ...
        self.reduce_dim = nn.Linear(3 + 384 + 384, 384)

        self.rebuild = SimpleRebuildFCLayer(384, step=8, hidden_dim=384)
        # self.ub2 = UpBlock(feat_dim = 512, down_rates = [1, 4, 2], knns = [16, 12, 8], up_factor = 8)

    def forward(self, p_c, f_c, coarse_points_list, head_list, global_feature):
        bs = p_c.shape[0]
        ...
        decode_head0 = self.head_fp0(p_c, coarse_sparse, None, head_sparse)
        p_c = p_c.transpose(2, 1).contiguous()
        p1, f0, p0_fps_idxs_122, _ = self.ub0(p_c, decode_head0, gf, [p_c, f_c], None, None)

        p1_t = p1.permute(0, 2, 1).contiguous()
        decode_head1 = self.head_fp1(p1_t, coarse_mid, None, head_mid)
        p2, _, _, _ = self.ub1(p1, decode_head1, gf, [p_c, f0], None, p0_fps_idxs_122)

        p2_t = p2.permute(0, 2, 1).contiguous()
        decode_head2 = self.head_fp2(p2_t, coarse_dense, None, head_dense)
        rebuild_feature = torch.cat([
            gf.unsqueeze(1).expand(-1, 2048, -1),
            decode_head2.transpose(2, 1),
            p2_t], dim=2)

        rebuild_feature = self.reduce_dim(rebuild_feature)  # B M C
        relative_xyz = self.rebuild(rebuild_feature).reshape(bs, 2048, 3, -1)  # B M 3 S
        rebuild_points = (relative_xyz + p2_t.unsqueeze(-1)).transpose(2, 3).reshape(bs, -1, 3)  # B N 3

        all_pc = [p1_t, p2_t, rebuild_points]
        return all_pc
...
class HGGNet(nn.Module):
    def __init__(self, model_configs):
        ...
        self.fp1 = PointNet_FP_Module(in_channel=384, mlp=[384, 384],
                                      use_points1=True, in_channel_points1=128)
        # self.proj = nn.Linear(fp_mlp[-1], emb_dim)
        self.transformer1 = TransformerDecoder(
            embed_dim=384, depth=2, num_heads=4, k=8, n_group=2, mlp_ratio=2,
        ...
        self.fp2 = PointNet_FP_Module(in_channel=384, mlp=[384, 384],
        ...
        self.coarse_reduce_dim = nn.Sequential(
            nn.Conv1d(in_channels=384, out_channels=128, kernel_size=1),
            nn.BatchNorm1d(128),
            nn.GELU()
        )
    ...
    def forward(self, ptc):
        ...
        decode_feature2 = self.transformer2(q=decode_feature1, v=v_2, q_pos=coarse, v_pos=stage1_points,
                                            denoise_length=None)
        decode_feature2 = decode_feature2.transpose(2, 1).contiguous()

        s12_down, coarse_down_2 = self.down12(decode_feature2, coarse, num=256)
        s2_fuse = self.featureFuse3(s12_down, up_feature_1)

        s23_down, coarse_down_1 = self.down23(s2_fuse, coarse_down_2, num=128)
        s3_fuse = self.featureFuse4(s23_down, feature)

        head_list = [s3_fuse, s2_fuse, decode_feature2]
        coarse_points_list = [coarse_down_1, coarse_down_2, coarse]

        coarse_feature = self.coarse_reduce_dim(decode_feature2)
        point_cloud_list = self.decoder(coarse, coarse_feature, coarse_points_list, head_list, global_feature)
