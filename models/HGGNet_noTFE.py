...
from models.model_utils import fps_sample_points, knn_point, CBL, Fold, \
...
class EdgeSA(nn.Module):
    def __init__(self, in_channel, out_channel, k=16):
        ...
        self.layer1 = nn.Sequential(nn.Conv2d(in_channel * 2, out_channel, kernel_size=1, bias=False),
...
class UpBlock(nn.Module):
    def __init__(self, feat_dim=384, global_dim=384, crt_dim=128, down_rates=None, knns=None, up_factor=4):
        ...
        self.inter_crt = CRT(dim_in=crt_dim, is_inter=True, down_rates=down_rates, knns=knns)
        self.intra_crt = CRT(dim_in=crt_dim, is_inter=False, down_rates=down_rates, knns=knns)
    ...
    def forward(self, p, head, gf, pre, fps_idxs_1, fps_idxs_2):
        bs = p.shape[0]
        h = self.pn(p, head, gf)
        g, fps_idxs_q1, fps_idxs_s1 = self.inter_crt([p, h], pre, fps_idxs_1, fps_idxs_2)
...
class HGGNet(nn.Module):
    def __init__(self, model_configs):
        ...
        self.encoder_1 = EdgeSA(in_channel=384, out_channel=512, k=8)
        self.encoder_2 = EdgeSA(in_channel=512, out_channel=384, k=8)
        self.encoder_3 = EdgeSA(in_channel=384, out_channel=384, k=8)
        ...
        # self.featureFuse3 = CAFFBlock(channel=256, channel2=128, out_channel=256)
        ...
        self.combine_mlp_2 = nn.Sequential(
            nn.Linear(512, 512),
            nn.GELU(),
            nn.Linear(512, 256),
            nn.GELU(),
        ...
        self.reduce_dim = nn.Linear(1027 + 256, 384)
        # self.decoder = Decoder()
        # self.decoder = Fold(in_channel=128, step=self.folding_step, hidden_dim=128)
        self.decoder = Fold(in_channel=384, step=4, hidden_dim=256)
    ...
    def forward(self, ptc):
        ...
        # edge_points = stage3_points.transpose(2,1).contiguous()
        x = self.input_proj(encoded_feature) + pe
        
        x1, _, _ = self.encoder_1(x.transpose(2,1), stage3_points, num=128)
        x, _, _ = self.encoder_2(x1, stage3_points,  num=128)
        x, _, _ = self.encoder_3(x, stage3_points, num=128)

        feature = x.transpose(1, 2).contiguous()
        global_feature = self.increase_dim(feature)  # B 1024 N
        global_feature = torch.max(global_feature, dim=1)[0]  # B 1024

        coarse = self.coarse_pred(global_feature).reshape(bs, -1, 3)
        coarse_inp = fps(ptc, 512)  # B 256 3
        ...
        up_feature_1 = self.fp1(stage1_points, stage3_points, stage1_feature, x)
        up_feature_c = up_feature_1.transpose(2, 1)
        combined = torch.cat([q, up_feature_c], dim=2)
        decode_feature_1 = self.combine_mlp(combined)
        ...
        up_feature_2c = up_feature_2.transpose(2, 1)
        combined_2 = torch.cat([decode_feature_1, up_feature_2c], dim=2)
        decode_feature2 = self.combine_mlp_2(combined_2)
        ...
        # s2_fuse = self.featureFuse3(s12_down, stage2_feature)

        s23_down, coarse_down_3, fps_idx_23 = self.down23(s12_down, coarse_down_2, num=128)
        # s3_fuse = self.featureFuse4(s23_down, feature)

        head_list = [s23_down, s12_down, decode_feature2_t]
        coarse_dense = coarse
        coarse_mid = coarse_down_2
        coarse_sparse = coarse_down_3

        coarse_inp = fps(ptc, 128)

        coarse_center = torch.cat([stage3_points, coarse_sparse, coarse_mid, coarse_dense], dim=1)
        feature_center = torch.cat([stage3_feature, s23_down, s12_down, decode_feature2_t], dim=2)

        rebuild_feature = torch.cat([
            global_feature.unsqueeze(1).expand(-1, 1024, -1),
            feature_center.transpose(2, 1),
            coarse_center], dim=2
        )  # B M 1027 + C
        rebuild3_feature = self.reduce_dim(rebuild_feature.reshape(bs * 1024, -1))  # B M C
        relative3_xyz = self.decoder(rebuild3_feature).reshape(bs, 1024, 3, -1)  # B M 3 S
        dense_points = (relative3_xyz + coarse_center.unsqueeze(-1)).transpose(2, 3).reshape(bs, -1, 3)  # B N 3

        ret = (coarse, dense_points)
