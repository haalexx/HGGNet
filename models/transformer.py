...
class decTransformer(nn.Module):
    ...
    def forward(self, in_pos, encode_features, guide_coding):
...
class decTransformer_self(nn.Module):
    def __init__(self, in_channels, out_channels, guide_channels, num_heads=8, qkv_bias=False,
                 qk_scale=None, attn_drop=0., proj_drop=0.):
        super(decTransformer_self, self).__init__()
        self.query_mlp = CBL(3+in_channels, out_channels)
        self.fuse_mlp = CBL(3+in_channels, out_channels)
        self.atten = decAttention(out_channels, out_channels, num_heads=num_heads, qkv_bias=qkv_bias,
                                  qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=proj_drop)
        self.out_mlp = CBL(out_channels, out_channels)

    def forward(self, in_pos, encode_features):
        npoints = in_pos.shape[2]
        # generate v
        # guide_coding = guide_coding.unsqueeze(-1).expand(-1, -1, npoints)
        # pos_features = self.pos_mlp(in_pos)
        qv = torch.cat((in_pos, encode_features), dim=1)
...
if __name__ == "__main__":
    enc_features1 = torch.randn(8, 1024, 64)
    global_fea1 = torch.randn(8, 1024)
    in_points1 = torch.randn(8, 3, 64)
    tran = decTransformer(1024, 1024, 1024)
    ...
    dec_points = ret1
    tran2 = decTransformer(512, 1024, 1024)
    ret2 = tran2(in_points2, enc_features2, global_fea1, ret1)
    print(ret2.shape)
