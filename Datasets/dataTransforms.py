                ...
                for k, v in data.items():
                    if k in objects and k in data:
                        if transform.__class__ in [
                            RandomRotatePoints, RandomScalePoints, RandomMirrorPoints, RandomSamplePoints
                        ]:
...
class Centeralize(object):
    ...
    def __call__(self, ptcloud):
        ...
        # 平移至原点
        points_centered = ptcloud - centroid
...
class RandomRotatePoints(object):
    ...
    def __call__(self, ptcloud, rnd_value):
        ...
        if rnd_value < 0.25:
            angle_value = np.random.uniform(0, 1)
            angle = 2 * math.pi * angle_value
...
class RandomScalePoints(object):
    ...
    def __call__(self, ptcloud, rnd_value):
        ...
        if rnd_value < 0.25:
            scale_value = np.random.uniform(0, 1)
            scale = np.random.uniform(1.0 / self.scale * scale_value, self.scale * scale_value)
